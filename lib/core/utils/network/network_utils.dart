...
//    _dio.interceptors.add(CookieManager(CookieJar()));
  ...
  Future requestDataFuture<T>(Method method, String url, String newBaseUrl,
    ...
    return await _request<T>(requestMethod, url,
      ...
      if (result.code == ErrorStatus.rEQUESTDATAOK || result.code == ErrorStatus.sUCCESS) {
        if (isList) {
        ...
        _onError(result.code ?? -1, result.message ?? "Error", onError!);
      }
    }, onError: (e) {
      //Logo.e(e.toString());
      _cancelLog(e, url);
  ...
  Future<BaseResponse<T>> _request<T>(String method, String url,
    ...
    var response = await _dio!.request(url,
        ...
        queryParameters: queryParameters,
        options: _setOptions(method, options ?? Options()),
        cancelToken: cancelToken);
    ...
    try {
      ...
      return BaseResponse(ErrorStatus.pARSEERROR, "Parsing Error: $e", null);
    }
  }
